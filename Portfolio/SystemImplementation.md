...
As one of the key foundations of Agile software development is to have self-organising teams, and we were confident in the capabilities to successfuly maintain a minimum viable product during each weeks sprints, we knew that it was important to collectively focus on the most important user stories to be implemented during each sprint. In order to keep this clear, we created a Gantt chart to be used between each of the teams working on the different subsystems. This was used in conjunction with our project's [Kanban Board](https://github.com/Team-Jag/Do-or-die-fitness-tracker/projects/1). Both of these features were useful to keep our work user focused, while ensuring cohesion between each subsystem. 
