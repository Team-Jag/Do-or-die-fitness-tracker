...
During our development process more and more of the functionality and code moved from the page specific classes to the MQTT class. While our initial "ideal" design would have the page specific content inside the respectiv page class, we faced the issue that all the variables/states are lost when navigating to the next page. Moving functionality to the MQTT class alowed us to retain the critical data. Furthermore the introduction of a global variable user_name allowed us to reatin the user name without the use of cookies.
As a downside our MQTT class is therefore rather complex and long, which is why we intended to refactor this. For example we discussed to have an abstract MQTT class with the basic communication functions in it and then have page specific MQTT classes (e.g. a "MQTT Profile Page" class, a "MQTT Challenge Selection Page" etc.) Unfortunately ![React does not support inheritance](https://reactjs.org/docs/composition-vs-inheritance.html) but only supports the composition approach that we followed in our design. While this design is not ideal/pure for as a long term solutions, it allows to rapidly build a functiong prototype. However, in future versions the designed should be reevaluated and breaking up the MQTT class should be considered i.e. trough the use of cookies and advanced React libaries (e.g. React Redux).
...
While we we are still convinced that React is the right choice the build the Website for our solution. Furture designs should consider the use of cookies and other React libaries (React Redux) should be considered to allow the retention of user data (Session ID stored in cookie) and break-down the MQTT class further. See more details on these consideration in our [Object Oriented Design: Web design Section](#web-design) and in the [Communication protocols: Desktop and web Section](#desktop-and-web)
