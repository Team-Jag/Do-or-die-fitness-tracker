...
// Some nasty functions here which need refactoring TODO
    ...
    void build_expanded_order(String orderid) {
      ...
      order.addItem(api.getUserByID(orderid).getString("current"), 0);

      ListBox order2 = cp5.addListBox("challenges")
        ...
        order.addItem(api.getUserByID(orderid).getString("upcoming"), 0);
        order.addItem(api.getUserByID(orderid).getString("current"), 1);
        order.addItem(api.getUserByID(orderid).getString("completed"), 2);
